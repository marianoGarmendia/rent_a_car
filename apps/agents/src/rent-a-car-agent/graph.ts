...
const toolNode = async (state: typeof stateGraph.State, config: LangGraphRunnableConfig) => {
  const { messages } = state;
...
const leadQualifierNode = async (state: typeof stateGraph.State ) => {
  ...
  if(leadQualifier && leadQualifier.calificado) return {
    messages: [...messages],}
  ...
  try {
    ...
    console.log(`📊 Lead calificado - Score: ${result.score}/10, Calificado: ${result.calificado}`);
